    ...
    def get_page_urls(self):
        if int(page_num) > 1:
            for n in range(2, int(page_num)+1):
                page_url = 'http://html body div.main div.pic ul li span.title a' + str(n)
...
# 获取图片的url的方法
    def get_pic_urls(self):
        driver = webdriver.Chrome(chrome_options=chrome_options)#对浏览器节进行配置
        for girl_url in self.girl_urls:
            driver.get(girl_url)
            time.sleep(3)#防止别人侦测到你是爬虫
